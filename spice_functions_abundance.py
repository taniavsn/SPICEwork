...
def fit_lines_SPICE(filename, data_path = "SPICE_files"):
        ...
        for i in tqdm_notebook(range(0,nx)):
            for j in range(0,ny):
                ...
                if(np.sum(np.logical_not(mask)) > 5):
                    ...
                    fit_err[i,j] = np.abs((g_fit.amplitude_0.value/sum(data[mask==0].value))*np.sqrt(sum((errs[mask==0])**2)))
    ...
    if nbr_keys == 3 :   
        #Intensities
        fig_intensity = plt.figure(constrained_layout=True)
        spec = fig_intensity.add_gridspec(ncols = 4, nrows = 1, width_ratios = [0.1, 0.3, 0.3, 0.3])
        
        ax1 = fig_intensity.add_subplot(spec[:,0])
        ax2 = fig_intensity.add_subplot(spec[0,1])
        ax3 = fig_intensity.add_subplot(spec[0,2])
        ax4 = fig_intensity.add_subplot(spec[0,3])
        
        # color bar param
        ax1.set(ylabel = 'Normalized intensity - adu')
        axes = [ax2, ax3, ax4]
        for i in range(len(keys)):
            axes[i].imshow((tot_fit_amps[i]/tot_fit_amps[i][:,200:630].max()),
                           vmax= 1, extent=[0,160*2.5,125,725],cmap='gist_heat')   
            #4:1 extent=[0,160*2.5,125,725]
            axes[i].set(title = str(keys[i]))
        clrbr = plt.colorbar()
        ax1.imshow(clrbr)
        plt.show()
        
        #Errors
        fig_err = plt.figure(constrained_layout=True)
        spec = fig_err.add_gridspec(ncols = 4, nrows = 1, width_ratios = [0.1, 0.3, 0.3, 0.3])
        
        ax1 = fig_err.add_subplot(spec[:,0])
        ax2 = fig_err.add_subplot(spec[0,1])
        ax3 = fig_err.add_subplot(spec[0,2])
        ax4 = fig_err.add_subplot(spec[0,3])
        
        # color bar param
        ax1.set(ylabel = 'Normalized errors - adu')
        axes = [ax2, ax3, ax4]
        for i in range(len(keys)):
            axes[i].imshow((tot_errors[i]/tot_errors[i][:,200:630].max()),
                           vmax= 1, extent=[0,160*2.5,125,725],cmap='gist_heat')   
            #4:1 extent=[0,160*2.5,125,725]
            axes[i].set(title = str(keys[i]))
        clrbr = plt.colorbar()
        ...
        spec = fig.add_gridspec(ncols = 3, nrows = 2, width_ratios = [0.1,0.45,0.45], height_ratios=[1,1])
...
def fit_contribution_DEM_abundance(filename, ions, wvl, data_path="SPICE_files"):
    ...
    for i in mixed_tresps :
        datasequences.append(emtk.em_data_spice(file, tot_fit_amps, tot_errors, logtsCorona, i))
